...
class TicketService {
    ...
    async processRefundChoice(ticketId, userId, action) {
        ...
        if (action === 'use_for_another_ticket') {
            ...
            const user = await models_1.User.findByPk(userId);
            await NotificationService_1.notificationService.sendNotification(userId, 'Bank Refund Requested', `Your refund of $${ticket.ticketSponsorshipRefundAmount || ticket.purchasePrice} has been queued for payout to your registered bank account (${user?.bankName || 'N/A'} - ${user?.accountNumber || 'N/A'}).`);
        }
        return ticket;
    }
    ...
    async submitReceipt(ticketId, userId, data) {
        const ticket = await models_1.Ticket.findOne({ where: { id: ticketId, userId } });
        if (!ticket)
            throw new Error('TICKET_NOT_FOUND');
        await ticket.update({
        ...
        for (const admin of admins) {
            await NotificationService_1.notificationService.sendNotification(admin.id, 'Payment Receipt Submitted', `Candidate has submitted a payment receipt for ${ticket.ticketType} (Ticket #${ticket.id}). Please verify and unlock course access.`);
        }
        return ticket;
    }
