...
async function registerCrons() {
    ...
    try {
        ...
        const listResponse = await axios.get(`https://qstash.upstash.io/v2/schedules`, {
        ...
        for (const schedule of existingSchedules) {
            await axios.delete(`https://qstash.upstash.io/v2/schedules/${schedule.scheduleId}`, {
                headers: { Authorization: `Bearer ${QSTASH_TOKEN}` }
            });
        ...
        for (const cron of crons) {
            ...
            const response = await axios.post(
                `https://qstash.upstash.io/v2/schedules/${destinationUrl}`,
                {},
                {
                    headers: {
