    ...
    async startAttempt(req, res, next) {
        try {
            ...
            if (error.message === 'PARTIAL_LIMIT_EXCEEDED')
                return res.status(403).json({ code: 403, error: 'PARTIAL_LIMIT_EXCEEDED', message: 'You have reached the limit of 3 ticket courses allowed under a partial payment. Please pay the remaining balance to unlock all courses.' });
            next(error);
        }
    }
