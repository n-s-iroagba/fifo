...
interface Ticket {
    ...
    User?: { id: number; fullName?: string; email: string; bankName?: string; accountNumber?: string; accountName?: string; avelingUsername?: string; avelingPassword?: string; };
...
export default function AdminTicketsPage() {
    ...
    const { data: bankRes, refetch: refetchBank } = useApiQuery<any>(
        ['platform-bank'],
        '/admin/platform-bank'
    );

    const [bankModalOpen, setBankModalOpen] = useState(false);
    const [platformBank, setPlatformBank] = useState({ bankName: '', bsb: '', accountNumber: '', accountName: '' });
    ...
    const handleSaveBank = async (e: React.FormEvent) => {
        ...
        setBankSaving(true);
        try {
            await api.put('/admin/platform-bank', platformBank);
            setBankModalOpen(false);
            refetchBank();
        } catch (err: any) {
            alert(err.response?.data?.message || 'Failed to save platform bank');
        } finally {
            setBankSaving(false);
        }
    };
    ...
    const handleGenerateCredentials = async () => {
        ...
        try {
            await api.post(`/admin/tickets/${selectedTicket.id}/generate-credentials`);
            setMessage('Aveling credentials generated and emailed to applicant.');
            refetch();
        } catch (err: any) {
    ...
    const handleValidatePayment = async () => {
        ...
        setActionLoading(true);
        try {
            await api.post(`/admin/tickets/${selectedTicket.id}/validate-payment`);
            setMessage('Payment verified. Course access unlocked and applicant notified.');
            refetch();
        } catch (err: any) {
                                    ...
                                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                                        ...
                                        {selectedTicket.receiptUrl && (
                                            <div className="p-3 bg-white border border-slate-200 rounded-lg">
                                                ...
                                                <a href={selectedTicket.receiptUrl} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline text-xs flex items-center gap-1">
                                                    <span className="material-symbols-outlined text-sm">open_in_new</span>
                                                    View Receipt Document (Ref: {selectedTicket.receiptReference})
                                                </a>
                                            </div>
                                        )}
                                        ...
                                        <button
                                            ...
                                            disabled={actionLoading || selectedTicket.paymentStatus === 'payment_verified'}
                                            className="w-full mt-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-[10px] uppercase tracking-widest py-3 rounded-xl transition-all"
                                        >
                                            {selectedTicket.paymentStatus === 'payment_verified' ? 'Payment Verified' : 'Verify Payment & Unlock Course'}
                                        </button>
                    ...
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-blue-100">
                        ...
                        <form onSubmit={handleSaveBank} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" value={platformBank.accountName} onChange={e => setPlatformBank({ ...platformBank, accountName: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-blue-900" placeholder="Aveling Training & FIFO" />
                            </div>

                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
                                <button type="button" onClick={() => setBankModalOpen(false)} className="px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:bg-slate-100">Cancel</button>
                                <button type="submit" disabled={bankSaving} className="bg-blue-900 hover:bg-blue-800 text-white px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg shadow-blue-900/10">
                                    {bankSaving ? 'Saving...' : 'Save Settings'}
                                </button>
                            </div>
                        </form>
                    </div>
