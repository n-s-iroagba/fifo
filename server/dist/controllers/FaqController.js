...
class FaqController {
    ...
    async getAll(req, res) {
        try {
            ...
            logger_1.logger.error('Failed to fetch FAQs:', error);
    ...
    async getById(req, res) {
        try {
            const { id } = req.params;
            const faq = await models_1.Faq.findByPk(Number(id));
            if (!faq) {
                res.status(constants_1.CONSTANTS.HTTP_STATUS.NOT_FOUND).json({
            ...
            logger_1.logger.error('Failed to fetch FAQ by ID:', error);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
    ...
    async create(req, res) {
        try {
            ...
            // Check total count
            const totalCount = await models_1.Faq.count();
            if (totalCount >= 2) {
                res.status(constants_1.CONSTANTS.HTTP_STATUS.BAD_REQUEST).json({
            ...
            const existing = await models_1.Faq.findOne({ where: { type } });
            if (existing) {
                res.status(constants_1.CONSTANTS.HTTP_STATUS.BAD_REQUEST).json({
            ...
            const faq = await models_1.Faq.create({
                type,
                link: link.trim(),
                title: title?.trim() || defaultTitle,
                description: description?.trim() || null,
            });
            logger_1.logger.info(`[FaqController] Created FAQ [id=${faq.id}, type=${faq.type}]`);
            ...
            logger_1.logger.error('Failed to create FAQ:', error);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
    ...
    async update(req, res) {
        try {
            ...
            if (type !== undefined) {
                ...
                if (type !== faq.type) {
                    const duplicate = await models_1.Faq.findOne({
                        where: {
                            type,
                            id: { [sequelize_1.Op.ne]: faq.id },
                        },
                    });
                    if (duplicate) {
                        res.status(constants_1.CONSTANTS.HTTP_STATUS.BAD_REQUEST).json({
            ...
            await faq.save();
            logger_1.logger.info(`[FaqController] Updated FAQ [id=${faq.id}, type=${faq.type}]`);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.OK).json({
                success: true,
            ...
            logger_1.logger.error('Failed to update FAQ:', error);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
    ...
    async delete(req, res) {
        try {
            ...
            await faq.destroy();
            logger_1.logger.info(`[FaqController] Deleted FAQ [id=${id}, type=${deletedType}]`);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.OK).json({
                success: true,
            ...
            logger_1.logger.error('Failed to delete FAQ:', error);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
    ...
    async upsert(req, res) {
        try {
            ...
            const defaultTitle = type === 'process' ? 'Process AI Assistant' : 'Payment AI Assistant';
            let faq = await models_1.Faq.findOne({ where: { type } });
            if (faq) {
                faq.link = link.trim();
                if (title !== undefined)
                    faq.title = title ? title.trim() : defaultTitle;
                if (description !== undefined)
                    faq.description = description ? description.trim() : null;
                await faq.save();
                logger_1.logger.info(`[FaqController] Upsert (updated) FAQ [id=${faq.id}, type=${faq.type}]`);
            }
            else {
                ...
                faq = await models_1.Faq.create({
                    type,
                    link: link.trim(),
                    title: title?.trim() || defaultTitle,
                    description: description?.trim() || null,
                });
                logger_1.logger.info(`[FaqController] Upsert (created) FAQ [id=${faq.id}, type=${faq.type}]`);
            }
            res.status(constants_1.CONSTANTS.HTTP_STATUS.OK).json({
                success: true,
            ...
            logger_1.logger.error('Failed to upsert FAQ:', error);
            res.status(constants_1.CONSTANTS.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
