...
export function Navbar() {
    ...
    const isPathBased = pathname.startsWith('/aveling');
    const prefix = isPathBased ? '/aveling' : '';

    const handleLogout = () => {
        localStorage.removeItem('lms_token');
        localStorage.removeItem('lms_user');
        router.push(prefix ? `${prefix}/login` : '/login');
    };

    if (pathname === '/login' || pathname === '/aveling/login') return null;

    const publicNavLinks = [
        { name: 'Home', href: prefix || '/', icon: Award },
        { name: 'Course Catalog', href: `${prefix}/catalog`, icon: BookOpen },
    ];

    const signedInNavLinks = [
        { name: 'Home', href: prefix || '/', icon: Award },
        { name: 'My Certifications', href: `${prefix}/my-certifications`, icon: Award },
        { name: 'Course Catalog', href: `${prefix}/catalog`, icon: BookOpen },
        { name: 'My Payments', href: `${prefix}/payments`, icon: CreditCard },
    ];

    const currentNavLinks = user ? signedInNavLinks : publicNavLinks;
                ...
                <div className="flex items-center gap-8">
                    ...
                    <nav className="hidden lg:flex lg:items-center lg:gap-1">
                        {currentNavLinks.map((link) => {
                            ...
                            const isActive = pathname === link.href || (link.href !== '/' && link.href !== '/aveling' && pathname.startsWith(link.href));
                            return (
                                <Link
                                    key={link.href}
                                    href={link.href}
