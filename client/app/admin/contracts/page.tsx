...
async function generateContractPDF(applicant: any, nomination: any, dateStr: string, tickets: any[] = []): Promise<Blob> {
    ...
    // Header
    addLine('Blue Collar Recruitment Pty Limited', 14, true, [0, 0, 128]);
    addSpace(2);
    addLine('FIFO EMPLOYMENT TICKETING, TRAINING & VISA SPONSORSHIP CANDIDATE AGREEMENT', 11, true);
    addLine('DOCUMENT REF: BCR-FIFO-2026-0810', 9, false, [80, 80, 80]);
    addLine('STRICTLY CONFIDENTIAL', 9, false, [80, 80, 80]);
    addLine(`DATE OF INSTRUMENT: ${dateStr}`, 9, false, [80, 80, 80]);
    addSpace(4);

    addLine('1. PARTIES', 11, true);
    addLine(`This Binding Agreement (hereinafter referred to as the "Agreement" or "Instrument") is made and entered into on this ${dateStr}, by and between:`);
    addLine('THE COMPANY: Blue Collar Recruitment Pty Limited (BC Recruit Pty Ltd) (ACN: 105 263 151 / ABN: 67 105 263 151), a licensed labor hire, workforce deployment, and recruitment enterprise duly incorporated under the laws of Australia, having its principal corporate headquarters located at Suite 213/35-37 Doody St, Alexandria, New South Wales, 2015, Australia (hereinafter referred to as "the Company").');
    addLine(`THE CANDIDATE: ${applicant.fullName}, an individual national of ${applicant.country || 'Guinea'}, currently residing within ${applicant.country || 'Guinea'}, holder of International Passport Number: ${applicant.passportNumber || '[____________________]'} (hereinafter referred to as "the Candidate").`);
    addLine('THE TRAINING PARTNER: Aveling (RTO Provider Code: 50503), an accredited Registered Training Organisation registered in the state of Western Australia, contracted by the Company to deliver industry-specific competency courses. The Candidate explicitly acknowledges that Aveling is a non-party to this Agreement, and its operational liabilities are governed separately under its autonomous educational enrollment framework.');
    addLine('Collectively, the Company and the Candidate shall be formally designated within this Instrument as "the Parties" and individually as "a Party."');
    addSpace(4);

    addLine('2. BACKGROUND & RECITALS', 11, true);
    addLine('WHEREAS:');
    addLine('The Company operates an international recruitment infrastructure specializing in sourcing, certifying, and deploying qualified candidates into domestic Fly-In Fly-Out (FIFO) industrial roles within the resources, mining, energy, and infrastructure sectors of the Commonwealth of Australia.');
    addLine('The Candidate has formally applied for structural career placement and requires specific vocational competencies, safety credentials, a verified skills assessment, and a lawful visa pathway to qualify for operational placement with the Company’s heavy-industry clients.');
    addLine('The Company possesses approved Standard Business Sponsor (SBS) status under the Migration Act 1958 (Cth) and is prepared to formally nominate the Candidate for international mobilization under the Skills in Demand (Subclass 482) temporary visa framework, subject to the Candidate’s reciprocal compliance with the academic, vocational, and chronological performance milestones established herein.');
    addSpace(4);

    addLine('3. DEFINITIONS & INTERPRETATION', 11, true);
    addLine('In this Agreement, unless the context otherwise requires, the following terms shall have the explicit meaning assigned to them:');
    addLine('“Ticket(s)” or “Certifications”: The industry-standard, nationally recognized units of competency, safety inductions, core hazardous area certifications, and resource sector entry programs itemized within Schedule 1 of this Instrument, systematically provisioned and verified by the designated Training Partner.');
    addLine('“TRA Assessment”: The formal offshore skills assessment framework administered by Trades Recognition Australia (TRA) to evaluate the Candidate’s technical trade qualifications and international employment history.');
    addLine('“OTSR (Offshore Technical Skills Record)”: The official credential issued by Trades Recognition Australia (TRA) upon successful completion of the offshore skills assessment, confirming that the Candidate meets the technical competencies required to apply for a provisional Australian trade licence.');
    addLine('“Sponsorship Contribution”: The explicit fiscal percentage allocation of eligible educational, administrative, and visa application outlays that the Company covenants to absorb or subsidize on behalf of the Candidate.');
    addLine('“Candidate Wallet”: An internal, verifiable corporate digital ledger maintained by the accounting infrastructure of the Company, serving to accurately record valid security deposits advanced by the Candidate alongside corresponding credits or success-based reimbursements accrued.');
    addLine('“Contract Return Date”: The calendar date upon which the Candidate returns a fully executed, un-amended copy of this complete Agreement to the authorized agents of the Company.');
    addLine('“Process”: The total integrated chronological sequence of milestones spanning the execution of this Instrument, initial capital deposits, completion of remote educational theory modules, TRA assessment clearances, medical screenings, lodging of immigration petitions, physical transit, and physical practical assessments, as structured under Schedule 2.');
    addLine(`“Nomination Position”: The specific, designated employment classification of ${nomination.tradeStream} at ${nomination.hostEmployer} – Fly-In Fly-Out (FIFO), mapped under the Australian and New Zealand Standard Classification of Occupations (ANZSCO) structural codes, deployed on behalf of the host client Company to be filled later. The placement will encompass high-yield operations inside Western Australia's mining regions.`);
    addLine('"Licensing Charges": The statutory regulatory fees enforced by the Western Australia Department of Transport (DoT) required to finalize local operating compliance, specifically consisting of the Computerised Theory Test (CTT), the Practical Driving Assessment (PDA), and the 1-Year Full Licence Card Fee.');
    addLine('"Assessment Attempt": A single, formal evaluation instance of an individual module or assessment block. Under Australian standards, this incorporates the baseline initial examination and, where required, an individual re-assessment event up to the permissible ceiling.');
    addSpace(4);
    ...
    addLine('4.3 Statutory Cost Exclusions: The Company explicitly guarantees that it shall not seek, directly or indirectly, to transfer, defer, claw back, withhold, or recover from the Candidate any regulatory costs which Australian migration law mandates must be borne exclusively by the sponsor. The Company shall pay one hundred percent (100%) of the following, without exception:');
    addLine('- Standard Business Sponsorship (SBS) application processing fees (A$420.00);');
    addLine('- Employer Nomination application processing fees (A$330.00);');
    addLine('- The full applicable Skilling Australians Fund (SAF) Levy (A$2,400.00);');
    addLine('- All corporate-level immigration legal fees, processing fees, or migration agency consultancy retainers relating to the sponsorship and nomination architecture.');
    addLine('4.4 Initial Mobilization Accommodation Support: The Company covenants to arrange, manage, and completely fund the Candidate\'s residential accommodation within the state of Western Australia for an initial, continuous duration of three (3) calendar months commencing immediately from the Candidate\'s physical arrival. This mobilization benefit is strictly capped at a maximum corporate expenditure threshold of fifteen thousand Australian dollars (A$15,000.00) and is explicitly contingent upon the Candidate remaining continuously engaged with the assigned client enterprise and avoiding any event of material breach or voluntary abandonment.');
    addSpace(4);

    addLine('5. CANDIDATE FINANCIAL OBLIGATIONS & DEPOSIT PROTOCOLS', 11, true);
    addLine('5.1 Initial Commitment Deposit: Prior to the booking, scheduling, or corporate funding of any individual online module or Certification block with the Training Partner, the Candidate shall transfer a security deposit of 250 USDT. This deposit serves as an essential instrument of mutual commercial assurance, confirming the Candidate’s earnest and binding commitment to complete the workflow and protecting the corporate capital of the Company from losses arising due to sudden, un-notified candidate withdrawal.');
    const candidateSub = (100 - Number(companySub)).toFixed(2);
    addLine(`5.2 Expense Caps & Balance Accountability: The Candidate retains absolute personal liability for the remaining ${candidateSub} percent (${candidateSub}%) balance of itemized Ticket and Induction training costs, 100% of individual regional Western Australian regulatory driving licensing outlays, and 100% of statutory fees levied by Trades Recognition Australia (TRA). The Candidate’s absolute cumulative financial exposure under this entire Instrument is strictly capped subject to downward adjustment upon the activation of success-based wallet credits as defined within Clause 7.`);
    addSpace(4);

    addLine('6. CANDIDATE PERFORMANCE MILESTONES & TIMELINE EXECUTION', 11, true);
    addLine('The Candidate explicitly covenants to execute every milestone detailed below within the rigorous parameters of the chronological timeline, which is summarized visually within Schedule 2. Time is of the essence in respect of all obligations contained inside this Clause:');
    addLine('6.1 Execution and Return Window: The Candidate must execute and return this complete, unaltered Agreement via authorized digital or physical channels within exactly twenty four (24) hours of receipt from the Company.');
    addLine('6.2 Identity and Passport Documentation: The Candidate must provide a certified, high-resolution digital color scan of their valid passport to the Company within a maximum duration of three (3) weeks calculated from the initial transmittal date of this Agreement.');
    addLine('6.3 Remote Theoretical Examination Delivery: The Candidate must formally enrol in, engage with, and complete the entirety of the remote, self-paced online theoretical components and associated educational assessments via the training partner portal while residing in Guinea. This explicitly encompasses the core theoretical units of the safety inductions and regular technical tickets. All such theoretical examination milestones must be finalized within two (2) calendar weeks of the Contract Return Date.');
    addLine('6.4 Arrival-Based Practical Assessments: The Candidate acknowledges that high-risk physical modules and mining induction criteria require in-person validation to comply with Australian safety frameworks. The Candidate covenants to present themselves at the designated training facility in Perth, Western Australia, to complete all physical demonstrations, safety scenarios, and practical equipment handling modules within the first seven (7) calendar days of arrival.');
    addLine('6.5 Regional Driving Readiness & Licensing Testing: The Candidate must present themselves to an official Western Australia Department of Transport (DoT) licensing location within fourteen (14) days of physical arrival to complete the Computerised Theory Test (CTT) and schedule the execution of their Practical Driving Assessment (PDA) to clear local driving authorizations.');
    addLine('6.6 Statutory Re-Examination & Assessment Retake Protocols: In alignment with Australian vocational education frameworks managed by the Australian Skills Quality Authority (ASQA) and Western Australian Department of Transport statutory criteria, assessment boundaries are rigorously capped to preserve structural competence standards:');
    addLine('6.6.1 Standard Initial Attempt and Retake Allowance: The Candidate is authorized a maximum of two (2) sequential assessment attempts in total to successfully clear any individual vocational ticket module, safety induction unit, technical trade clearance exam, or DoT driving evaluation stream.');
    addLine('6.6.2 First Attempt Retake Management: In the event that the Candidate fails to secure a passing grade on their initial attempt of any assessment, they must instantly activate standard remediation processes. The single permissible second attempt (retake) must be executed within forty-eight (48) hours for remote online theory modules, or within a timeline dictated directly by the local testing administrator for practical and driving streams.');
    addLine('6.6.3 Strict Binary Threshold: The Candidate is explicitly prohibited from pursuing a third (3rd) attempt or any further continuous re-testing inside the parameters of this Agreement\'s financial structures. A secondary consecutive failure represents an automatic academic default event.');
    addLine('6.7 Aggregate Process Completion: The entire preliminary workflow spanning contract return, online theoretical certifications, TRA skills clearance, medical results, and visa-readiness must be executed in full within one (1) single calendar month of the Contract Return Date, unless a formal extension is granted by the Company in writing.');
    addLine('6.8 Interview Protocols: The Candidate must make themselves available for a mandatory screening and operational phone interview with the Company’s recruitment leadership immediately following the verifiable completion and passing of their first online Certification block.');
    addLine('6.9 Mobilization Readiness: The Candidate must maintain constant logistical readiness to execute immigration application documents and initiate international transit to Australia within four (4) weeks of reaching visa-ready status, subject only to sovereign Department of Home Affairs processing backlogs outside human control.');
    addLine('6.10 Medical Fit-for-Work & Screening Protocols: The Candidate must completely satisfy a dual-tiered health validation architecture, split across statutory immigration requirements and regional heavy-industry occupational safety mandates. Passing outcomes in both areas represent an absolute, non-negotiable prerequisite for visa issuance, mobilization, and site deployment:');
    addLine('6.10.1 Subclass 482 Visa Medical Requirements (Immigration Path): Concurrently with the lodgement of the visa application, the Department of Home Affairs will trigger mandatory offshore health examinations based on the Candidate’s nationality. These must be performed by an authorized panel physician inside an approved global affiliate clinic and consist of:');
    addLine('- Chest X-ray (Form 160): Required for candidates aged 11 and older to satisfy clearing criteria for active Tuberculosis (TB).');
    addLine('- Medical Examination (Form 501): A standardized full general physical assessment executed by the panel physician.');
    addLine('- Serum Creatinine / eGFR Blood Test: Required for candidates over 15 years of age to explicitly clear kidney function safety baselines.');
    addLine('Scope Note: Because the Candidate is being nominated for a technical trade infrastructure role rather than a healthcare, childcare, or classroom educational teaching stream, tests for HIV or Hepatitis will not be automatically generated for this temporary visa subclass.');
    addLine('6.10.2 Industry-Specific FIFO Medical Requirements (Employer Path): Fully independent of the statutory visa path, Western Australia\'s mining sector mandates strict, non-negotiable physical clearings. The Company or its industrial client shall deploy the Candidate to a specific commercial occupational health provider (scheduled upon physical arrival in Perth or at a designated domestic hub) to complete a comprehensive fit-for-work evaluation consisting of:');
    addLine('- Instant Drug & Alcohol Screening: Absolute zero-tolerance urine and breath analysis screening, performed during the baseline medical and subject to ongoing random on-site testing.');
    addLine('- Audiometric Testing: A baseline hearing evaluation to systematically establish sensory baselines for long-term tracking around site heavy machinery.');
    addLine('- Spirometry: Comprehensive lung-function tracking to assess performance resilience against regional mine dust exposure.');
    addLine('- Functional Capacity Assessment: Specialized physical strength, ergonomic endurance, and lifting checks to verify the Candidate can safely manage demanding 12-hour swing roster shifts.');
    addLine('6.11 Integrity of Disclosures: The Candidate must provide strictly authentic, accurate, comprehensive, and untampered documentation throughout the lifecycle of this agreement. The Candidate is under a continuous legal obligation to instantly notify the Company of any material shift in personal circumstances, including hidden medical conditions, criminal convictions, charges, or historical visa rejections from any global sovereign jurisdiction.');
    addLine('6.12 Safe Harbour for Unforeseen Delays: In the event that a milestone cannot be met due to a verifiable force majeure event or standard external disruption (such as severe physical illness, certified governmental processing gridlocks, or unexpected technical outages on the educational network), the Candidate must instantly notify the Company. The Parties agree to negotiate a reasonable chronological variation in good faith, and such disruptions shall not be deemed a contract breach.');
    addLine('6.13 Offshore Technical Skills Recognition (TRA Pathway): The Candidate must formally complete an offshore skills assessment through Trades Recognition Australia (TRA) while operating from their home jurisdiction. It is a strict material condition of employment that the Candidate successfully obtains an Offshore Technical Skills Record (OTSR). This record represents the absolute baseline necessary to verify structural trade equivalence for temporary immigration sponsorship and to facilitate the rapid onboarding of a provisional Western Australian operating licence upon deployment. All administrative and statutory assessment fees imposed directly by the TRA remain the exclusive out-of-pocket responsibility of the Candidate.');
    addSpace(4);

    addLine('7. THE CANDIDATE WALLET & SUCCESS-BASED REFUNDS', 11, true);
    addLine('7.1 Systematic Recrediting Architecture: To incentivize performance and eliminate structural risk, the Parties agree that upon the Candidate achieving verified passing marks for any individual Ticket module or safety induction, the corresponding personal contribution or advance deposit previously transferred by the Candidate for that specific block shall be automatically re-credited into the Candidate Wallet at a value of one hundred percent (100%).');
    addLine('7.2 Liquidity and Optionality: The Candidate holds absolute title and ownership over all capital resting within the Candidate Wallet. At their complete, un-fettered discretion, the Candidate may: (a) execute an immediate withdrawal of the liquid funds, or (b) elect to strategically assign the accrued credit balance to offset the remaining candidate liability associated with subsequent Tickets or the pending visa application charge.');
    addLine('7.3 Forfeiture Exclusions: Wallet balances derived from completed and passed modules remain the un-alienable property of the Candidate and can never be subject to corporate forfeiture, claw-back, or punitive retention, except in the highly specific default conditions explicitly laid out under Clause 9.');
    addLine('7.4 Governance and Accountability: The Company’s financial compliance division shall maintain precise ledger transparency and shall issue an official itemized balance statement to the Candidate within forty-eight (48) hours of receiving a written request.');
    addSpace(4);

    addLine('8. EXPRESS COVENANTS OF THE COMPANY', 11, true);
    addLine('The Company pledges its corporate infrastructure to the following clear performance parameters:');
    addLine(`8.1 Timely Financial Disbursement: The Company shall systematically advance its required ${companySub} percent (${companySub}%) corporate subsidy to the training infrastructure immediately upon the Candidate fulfilling the corresponding deposit baseline, preventing any artificial processing delays.`);
    addLine('8.2 Absolute Fiscal Transparency: The Company shall furnish comprehensive, itemized written breakdowns of all real course costs, baseline deposit tracking, and SWIFT international bank transfer routing protocols prior to issuing any payment demand to the Candidate.');
    addLine('8.3 Prompt Interview Execution: The Company shall coordinate and execute the required candidate screening and operational phone interview within seventy-two (72) hours of the Candidate passing their initial online course module.');
    ...
    addLine('- Channel Architecture: All incoming candidate transfers must execute strictly via standard International Wire Transfer (SWIFT Wire) using the verified routing coordinates issued exclusively within our corporate invoice paperwork. Independent third-party payment platforms or cash handovers are rejected.', 9);
    addLine('- Settlement Schedule: The foundational 250 USDT deposit is required immediately post-signing to secure the primary instructional allocations. Partial Payment above 50% of the above stated amount can be made with mandatory completion after 3 tickets.', 9);
    addSpace(4);

    addLine('SCHEDULE 2 — STRUCTURAL MILESTONES & EXECUTION DEADLINES', 11, true);
    ...
    addLine('15. CONTRACTUAL EXECUTION & SIGNATURES', 12, true);
    addLine(`IN WITNESS WHEREOF, the Parties hereto have caused this Candidate Agreement to be duly executed by their respective authorized signatures, creating a binding, reciprocal legal instrument effective as of ${dateStr}.`);
