...
export class TicketService {
    public async getUserTickets(userId: number) {
        ...
        return tickets.filter((t: any) => {
            if (!t.Application) return true; // Global ticket or old data
            const stages = t.Application.JobStages || [];
            return stages.some((s: any) => s.name === 'TicketSponsorship' || s.name === 'Ticket Sponsorship');
        });
    }
    ...
    public async createAndSendCustomInvoice(userId: number, data: {
        ...
        await notificationService.sendNotification(
            ...
            `An invoice of A$${data.amountAud.toFixed(2)} (${data.currency} ${data.convertedAmount.toFixed(2)}) has been sent to your email with payment details.`
        );
    ...
    public async processAvelingInvoicePayment(userId: number, invoicePurpose: string) {
    ...
    public async approveSponsorshipPackage(userId: number) {
        ...
        await notificationService.sendNotification(
            ...
            `Your ticket sponsorship application has been approved by Blue Collar Recruitment! You will receive your training invoice from Aveling shortly.`
        );
